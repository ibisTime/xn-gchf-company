...
import { UPLOAD_URL, PIC_PREFIX, PIC_BASEURL_L, tailFormItemLayout, tailFormItemLayout1,
  ...
  DATETIME_FORMAT, TIME_FORMAT } from 'common/js/config';
...
import CTime from 'component/cTime/cTime';
...
export default class DetailCompDev extends React.Component {
  ...
  getItemByType(type, item) {
    ...
    switch (type) {
      ...
      case 'time':
        return this.getTimeItem(item, initVal, rules, getFieldDecorator);
      case 'img':
  ...
  getTimeItem(item, initVal, rules, getFieldDecorator) {
    const props = {
      rules,
      initVal,
      getFieldDecorator,
    ...
    return <CTime key={item.field} {...props} />;
  }
  // 获取范围日期选择类型的控件
  ...
  beforeSubmit(err, values) {
    ...
    fields.forEach(v => {
      ...
      } else if (v.type === 'date' || v.type === 'datetime' || v.type === 'month' || v.type === 'time') {
        let format = v.type === 'date' ? DATE_FORMAT : v.type ===
          'month' ? MONTH_FORMAT : v.type === 'time' ? TIME_FORMAT : DATETIME_FORMAT;
        if (v.rangedate) {
